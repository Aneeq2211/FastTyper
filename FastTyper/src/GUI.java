...
public class GUI extends JFrame implements KeyListener, ItemListener {
    private final int WIDTH = 800, LENGTH = 800;
    Font promptFont;
    private JTextArea promptBox, typeBox;
    private JLabel wrongLabel, numErrorsLabel, accuracyLabel, wpmLabel, bglabel;
    ...
    public GUI() {
        ...
        promptFont= new Font("Times New Roman",Font.PLAIN,16);
        bglabel=new JLabel("Sakshi");
        bglabel.setBackground(Color.blue);
        add(bglabel);
        errors = new HashMap<Character, Integer>();
        qwerty = "qwertyuiopasdfghjkl;'zxcvbnm,./ ";
        prompts = new String[] {
                "The average typing speed of a computer typist is just 36wpm." +
                        " Touch typists are generally faster, averaging a quick 48 words per minute." +
                        " The fastest typing speed ever recorded is currently an extreme 216 words per minute! Average accuracy is 92%." +
                        " Bananas?",
                "\"The quick brown fox jumps over the lazy dog\" is a phrase that's commonly used to showcase fonts," +
                        " as it contains every letter of the english language, making it a pangram." +
                        " My personal favorite is: \"You go tell that vapid existentialist quack Freddy Nietzsche that he can just bite me, twice.\"",
                "Okay, we're finally down to the last prompt." +
                        " I'm running out of things to say." +
                        " Maybe I could give you more amazing typing fun facts? " +
                        "Or write you a quick, but majestic poem?" +
                        " A story about the quick brown fox. Perhaps I could write about special characters like / or *. Bye!"
        };
        promptNum = 0;
        ...
        promptBox.setBackground(Color.orange);
        promptBox.isOptimizedDrawingEnabled();
        promptBox.getFont();
        promptBox.setLineWrap(true);
        promptBox.setWrapStyleWord(true);

        typeBox.setSize(500, 100);
    ...
    public class BGcolor extends Applet{
        public void init()
        {
                GUI.this.setBackground(Color.blue);
        }
    }
    ...
    public void addSpacebar() {
        ...
        spacebar.setSize(400, 50);
        spacebar.setLocation(240, 500);
    ...
    public void setPosition(int i) {
        ...
        if (i <= 9) {
            buttons.get(i).setLocation(190 + 50*i, 350);
            buttons.get(i).setBackground(Color.CYAN);
        }
